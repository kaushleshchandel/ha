...
rotary_encoder::RotaryEncoderSensor *ren;
...
display::DisplayPage *page1;
display::DisplayPageShowNextAction<> *display_displaypageshownextaction;
display::DisplayPage *page2;
...
void setup() {
  ...
  ren = new rotary_encoder::RotaryEncoderSensor();
  App.register_component(ren);
  // time.homeassistant:
  //   platform: homeassistant
  //   id: esptime
  ...
  //         id: my_display
  //       type_id: display_displaypageshownextaction
  //     - component.update:
  //         id: my_display
  ...
  App.register_sensor(ren);
  ren->set_name("Rotary Encoder");
  ren->set_unit_of_measurement("steps");
  ren->set_icon("mdi:rotate-right");
  ren->set_accuracy_decimals(0);
  ren->set_force_update(false);
  mqtt_mqttsensorcomponent = new mqtt::MQTTSensorComponent(ren);
  ...
  page1 = new display::DisplayPage([=](display::DisplayBuffer & it) -> void {
      it.print(0, 10, font1, "This is page 1!");
  });
  display_displaypageshownextaction = new display::DisplayPageShowNextAction<>(my_display);
  alarm_state->set_entity_id("alarm_control_panel.my_alarm_system");
  ren->set_pin_a(new GPIOPin(14, INPUT, false));
  gpio_gpioswitch->set_pin(new GPIOPin(13, OUTPUT, false));
  gpio_gpioswitch->set_restore_mode(gpio::GPIO_SWITCH_RESTORE_DEFAULT_OFF);
  page2 = new display::DisplayPage([=](display::DisplayBuffer & it) -> void {
      it.printf(64, 0, font1, TextAlign::TOP_CENTER, "Alarm State: %s", alarm_state->state.c_str());
  });
  my_display->set_pages({page1, page2});
  ren->set_pin_b(new GPIOPin(12, INPUT, false));
  ren->set_resolution(rotary_encoder::ROTARY_ENCODER_1_PULSE_PER_CYCLE);
  ren->set_min_value(10);
  ren->set_max_value(50);
  my_display->set_model(ssd1306_base::SH1106_MODEL_128_32);
  my_display->set_brightness(1.0f);
  ...
  automation->add_actions({display_displaypageshownextaction, updatecomponentaction});
